...
const UpLocName = () => {
  ...
  const add = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch("http://localhost:8080/updatelocname", {
        method: "POST",
        headers: {
      ...
      if (data) {
        window.alert("location name updated");
      } else {
  ...
  return (
    <div className="grow rounded-md border border-gold p-4">
      <p className="mb-2 font-thin capitalize">update location name</p>
      <form onSubmit={add}>
