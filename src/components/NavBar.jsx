...
const NavBar = ({ cart }) => {
  ...
  return (
    <nav className="required: relative z-10 flex items-center justify-between border-b-4 border-gold bg-custom1 px-8 py-5 shadow-xl">
      ...
      <ul
        className={`md:py-0" absolute left-0 top-[84px] flex ${
          visble ? "scale-y-100" : "scale-y-0"
        } w-fit flex-col justify-center space-y-4 bg-main px-7 py-5 md:visible md:static md:scale-y-100 md:flex-row md:items-center md:space-x-8 md:space-y-0 md:bg-transparent md:px-0`}
        id="nav-bar"
      >
        <Link to="/">
          <li
      ...
      <div className="flex items-center space-x-3 text-white ">
        <Link to="/cart">
          <button className="relative w-fit block">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
            ...
            <p className="absolute -top-2 -right-2 bg-red-500 size-7 grid place-items-center rounded-full">
              {cart.length}
            </p>
          </button>
        </Link>
